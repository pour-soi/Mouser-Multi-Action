...
Generic Mouse Mode 让使用 Windows 标准侧键的普通鼠标也能使用 PourInput，无需依赖 Logitech HID++。支持的侧键可以分别设置单击和长按操作；关闭该模式后，侧键会恢复原本的返回和前进功能。
...
- **Generic Mouse Mode 支持 Windows 标准侧键**：使用标准 Windows XBUTTON 侧键事件的鼠标可以在不依赖 Logitech HID++ 的情况下使用 PourInput；该功能已通过 ZOWIE 鼠标手动验证。
- **Generic Mouse Mode 运行时开关**：关闭该模式时，PourInput 会移除活动的 XBUTTON 回调和拦截，使浏览器返回 / 前进恢复原生行为，无需重启应用。
...
- Generic XBUTTON1：单击 -> Copy，长按 -> Browser Forward
- Generic XBUTTON2：单击 -> Paste，长按 -> Browser Back
...
Generic Mouse Mode 是 Windows 专用模式，面向以 XBUTTON1 和 XBUTTON2 形式出现的标准鼠标侧键。它不需要 Logitech HID++，因此当非 Logitech 鼠标的侧键报告为 Windows 标准 XBUTTON 事件时，也可以使用 PourInput 动作。

当前实现只适用于 Windows 标准 XBUTTON 侧键事件。它支持单击和长按动作，支持运行时开启 / 关闭，并且在关闭后恢复原生返回 / 前进行为。

这不代表 PourInput 已经支持所有鼠标、所有鼠标按键、所有操作系统，也不支持那些不会显示为 Windows 标准 XBUTTON 事件的厂商专有按键。PourInput 目前也还不能可靠地区分多只标准鼠标上的侧键来源。

## 支持设备

PourInput 采用 capability-based device support（基于设备能力的支持）思路。也就是说，PourInput 会根据设备实际具备的能力开放相应功能，而不是简单地按照品牌判断“支持”或“不支持”。例如，带有 Windows 标准侧键的鼠标可以使用 Generic Mouse Mode，而受支持的 Logitech 设备还可以使用额外的 HID++ 功能。
...
| 设备 | 状态 |
|------|------|
| 带 Windows 标准侧键的 ZOWIE 鼠标 | 已在 Windows 上通过 Generic Mouse Mode 手动验证 |
| MX Master 3 | 已测试已编目的 Multi-Action 控件和 HID++ 能力检测 |

### 实验性 / 潜在兼容设备

以下设备不视为官方已支持，除非已经在 PourInput 中完成实际测试。它们在暴露匹配的 Windows 标准 XBUTTON 事件或 HID++ 能力时可能可用。

| 设备 | 说明 |
|------|------|
| 带 XBUTTON 侧键的 Windows 标准鼠标 | 可能可通过 Generic Mouse Mode 使用侧键单击和长按动作 |
...
Multi-Action 支持适用于 Generic Mouse Mode 侧键，也适用于已暴露对应控件的受支持 Logitech 设备。DPI、SmartShift、电量、手势控制和水平滚动等设备能力会因设备和固件而异。
...
- Generic Mouse Mode 目前只支持 Windows 标准 XBUTTON1 / XBUTTON2 侧键事件。
- Generic Mouse Mode 目前还不能按设备区分多只标准鼠标。
- 不会显示为 Windows 标准 XBUTTON 事件的厂商专有按键不属于 Generic Mouse Mode 的支持范围。
...
- **Generic Mouse Mode**：Windows 标准 XBUTTON 侧键可以在不依赖 Logitech HID++ 的情况下使用 PourInput 动作；关闭后恢复原生返回 / 前进行为。
